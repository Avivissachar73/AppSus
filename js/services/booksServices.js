...
const BOOKS_STORAGE_KEY = 'bookShop';
...
function getGoogleBooks(searchStr) {
  return getBooksFromApi(GOOGLE_BOOKS_API + searchStr)
          .then(data => {
            gApiBooks = data.items.reduce((acc, book) => {
                  book.volumeInfo.id = book.id;
                  if (book.volumeInfo.imageLinks.thumbnail) book.volumeInfo.thumbnail = book.volumeInfo.imageLinks.thumbnail;
                  else  book.volumeInfo.thumbnail = 'https://www.quantabiodesign.com/wp-content/uploads/No-Photo-Available.jpg';
                  if (book.saleInfo.listPrice) book.volumeInfo.listPrice = book.saleInfo.listPrice;
                  else book.volumeInfo.listPrice = 
                  {
                    "amount": 100,
                    "currencyCode": "EUR",
                    "isOnSale": false
...
function getBooksFromApi(API) {
  return fetch(API)
          .then(data => data.json()
              .then(books => books));
}

function getDiffBookIdById(diff, bookId) {
  var bookIdx = gBooks.findIndex(book => book.id === bookId);
  bookIdx += diff;
  if (bookIdx >= gBooks.length) bookIdx = diff-1;
  if (bookIdx < 0) bookIdx = gBooks.length+diff;
  
  return gBooks[bookIdx].id;
}

function removeReview(bookId, reviewId) {
  return getBookById(bookId)
    .then(book => {
      let reviewIdx = book.reviews.find(review => review.id === reviewId);
...
function getBookById(bookId) {
  return getBooks().then(someBooks => gBooks = someBooks)
    .then(() => {
...
function getBooks() {
  return new Promise((resolve, reject) => {
    utils.loadFromLocalStorage(BOOKS_STORAGE_KEY)
      .then(books => {
        resolve(books)
      })
      .catch(err => {
